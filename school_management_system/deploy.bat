@echo off
REM Script to help with deployment to Render.com

echo Generating a secure random key...
REM Generate a random key using PowerShell
for /f "tokens=*" %%a in ('powershell -Command "[System.Convert]::ToBase64String([System.Security.Cryptography.RandomNumberGenerator]::Create().GetBytes(32))"') do set SECRET_KEY=%%a
